...
function update_property_general!(iw::InterpolationWeight, wls::WLS, pt, mesh::CartesianMesh, filter::AbstractArray{Bool})
    indices = neighboringnodes(iw)
    kernel = get_kernel(wls)
    ...
    if filter isa Trues && get_device(mesh) isa CPUDevice # Fast path is not currently supported on GPU
        ...
        if dim == 1
            ...
            vals_axes = ntuple(Val(dim)) do d
                ...
                prop_1d = create_property(Val((kernel_support(wls),)), Vec{1,T}, wls_1d; derivative=order)
