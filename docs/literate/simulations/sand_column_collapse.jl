...
@test mean(sand_column_collapse(QuadraticBSpline(),                   FLIP(); test=true).x) ≈ [0.0011849337981031125, 0.13329422294040896]  rtol=1e-5 #src
@test mean(sand_column_collapse(QuadraticBSpline(),               FLIP(0.95); test=true).x) ≈ [0.0010858613366582644, 0.1443448693922295]   rtol=1e-5 #src
@test mean(sand_column_collapse(uGIMP(),                              FLIP(); test=true).x) ≈ [-0.0020039675305209164, 0.13840627541575792] rtol=1e-5 #src
@test mean(sand_column_collapse(KernelCorrection(QuadraticBSpline()), TPIC(); test=true).x) ≈ [2.557154039680512e-5, 0.13298463143093403]   rtol=1e-5 #src
@test mean(sand_column_collapse(KernelCorrection(QuadraticBSpline()), APIC(); test=true).x) ≈ [0.0003389619414787667, 0.13282160589432374]  rtol=1e-5 #src
@test mean(sand_column_collapse(LinearWLS(QuadraticBSpline()),        TPIC(); test=true).x) ≈ [-2.3507458570176305e-5, 0.13533560079308682] rtol=1e-5 #src
@test mean(sand_column_collapse(LinearWLS(QuadraticBSpline()), WLSTransfer(); test=true).x) ≈ [-2.3507458570180574e-5, 0.13533560079308687] rtol=1e-5 #src
end                                                                                                                                                   #src
