...
This is the usual CPU path when [`Partition`](@ref) is already used for threaded scattering.
...
On GPU, `update_sparsity!` takes particle positions, not a partition.
...
The block size is controlled by the `block_size_log2` option of [`CartesianMesh`](@ref), and the same block decomposition is used by [`Partition`](@ref):
