@testset "SpIndex" begin
    @test isactive(Sequoia.SpIndex(1, 0)) === false
    @test isactive(Sequoia.SpIndex(1, 1)) === true
    @test isactive(Sequoia.SpIndex(1, 2)) === true
    @test isactive(Sequoia.SpIndex(CartesianIndex(1,1), 0)) === false
    @test isactive(Sequoia.SpIndex(CartesianIndex(1,1), 1)) === true
    @test isactive(Sequoia.SpIndex(CartesianIndex(1,1), 2)) === true
    A = rand(3,3)
...
@testset "SpIndices" begin
    ...
    @test blocksize(spinds) === blocksize((12,20))
    @test !all(isactive, spinds)
    @test !all(i->isactive(spinds,i), eachindex(spinds))
    blkspy = rand(Bool, blocksize(spinds))
...
@testset "SpArray" begin
    ...
    @test size(A) === (12,20)
    @test !all(i->isactive(A,i), eachindex(A))
    @test all(iszero, A)
    blkspy = rand(Bool, blocksize(A))
    n = update_block_sparsity!(A, blkspy)
    @test length(Sequoia.get_data(A)) === n
    @test all(i->(A[i]=rand(); iszero(A[i])), filter(i->!isactive(A,i), eachindex(A)))
